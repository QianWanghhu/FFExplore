...
f_sobol = f_read(f'{f_dir[1]}{f_names[0]}.csv')
conf_morris = f_read(f'{f_dir[0]}{f_names[1]}.csv')
conf_sobol = f_read(f'{f_dir[1]}{f_names[1]}.csv')
mae_comp = pd.DataFrame(index=f_morris.index)
mae_comp['Morris'] = f_morris[sample_size[0]]
mae_comp['Sobol'] = f_sobol[sample_size[1]]
mae_comp['Morris_low'] = mae_comp['Morris'] - conf_morris[sample_size[0]]
mae_comp['Morris_up'] = mae_comp['Morris'] + conf_morris[sample_size[0]]
mae_comp['Sobol_low'] = mae_comp['Sobol'] - conf_sobol[sample_size[1]]
mae_comp['Sobol_up'] = mae_comp['Sobol'] + conf_sobol[sample_size[1]]
...
ax.plot(mae_comp['Sobol'][8:9], 'd', color='blue', alpha=0.5, ms=7,
        label='Number of fixed parameters identified')
...
ax.set_xlabel('The number of parameters fixed', fontsize=14)
ax.set_ylabel('RMAE (%)', fontsize=14)
ax.legend(['Morris (1980)', 'Sobol (18400)',
            'Number of fixed parameters identified',
            'Threshold (6%)',
            f'95% CIs for Morris',
            f'95% CIs for Sobol'], 
            title='GSA method (sample size)', fontsize = 10);
